...
from .serializers import JobPostSerializer, ApplyingUserSerializer
...
class ApplyingUserView(APIView):
    permission_classes = [IsCandidateUser]

    def post(self, request):
        request.data['user'] = request.user.id

        applying_user_serializer = ApplyingUserSerializer(data=request.data)

        if applying_user_serializer.is_valid():
            applying_user_serializer.save()
            return Response(status=status.HTTP_200_OK)
        
        return Response(applying_user_serializer.errors, status=status.HTTP_400_BAD_REQUEST)